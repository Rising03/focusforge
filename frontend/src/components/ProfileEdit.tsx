...
import { UserProfile, ProfileFormData, EnergyLevel } from '../types/profile';
...
type Section = 'identity' | 'goals' | 'schedule' | 'energy';

const ProfileEdit: React.FC<ProfileEditProps> = ({ onSave, onCancel }) => {
  const [currentSection, setCurrentSection] = useState<Section>('identity');
  ...
  // Predefined options (same as ProfileSetup)
  ...
  const sections = [
    { key: 'identity' as Section, label: 'Identity', icon: '👤' },
    { key: 'goals' as Section, label: 'Goals', icon: '🎯' },
    { key: 'schedule' as Section, label: 'Schedule', icon: '⏰' },
    { key: 'energy' as Section, label: 'Energy', icon: '⚡' },
  ];
  ...
  const loadProfile = async () => {
    try {
      ...
      // Parse identities from "Identity 1 + Identity 2" format
      const identities = profile.target_identity.includes(' + ') 
        ? profile.target_identity.split(' + ')
  ...
  const handleSave = async () => {
    ...
    try {
      console.log('ProfileEdit: Saving profile with data:', formData);
      const response = await profileService.updateProfile(formData);
      console.log('ProfileEdit: Save response:', response);
      console.log('ProfileEdit: Response profile wake_up_time:', response.profile.wake_up_time);
      console.log('ProfileEdit: Response profile sleep_time:', response.profile.sleep_time);
      
      setHasUnsavedChanges(false);
      
      // Reload profile to ensure we have the latest data from the server
      await loadProfile();
    ...
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">
          Your Target Identities
        </h2>
        <p className="text-gray-600 dark:text-slate-400">
          Who do you want to become? Select one or more identities.
        </p>
      </div>
      ...
      <div>
        ...
        {selectedIdentities.length > 0 && (
          <div className="mt-3 p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
            <p className="text-sm text-gray-600 dark:text-slate-400 mb-2">Your Target Identities:</p>
  ...
  const renderScheduleSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 dark:text-slate-400">
          Update your daily rhythm and available time.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label htmlFor="wake_up_time" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
            Wake Up Time
          </label>
          <input
            type="time"
        ...
        <div>
          <label htmlFor="sleep_time" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
            Sleep Time
          </label>
          <input
            type="time"
      ...
      <div>
        <label htmlFor="available_hours" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
      ...
      <div className="space-y-6">
        ...
        {timeSlots.length > 0 ? (
          ...
          <div className="text-center p-8 bg-gray-50 dark:bg-white/5 rounded-lg">
            <p className="text-gray-600 dark:text-slate-400">
              Please set your wake up and sleep times in the Schedule section.
            </p>
          </div>
        )}
      </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white dark:bg-slate-900 rounded-lg shadow-lg p-8">
                ...
                <button
                  ...
                  className={`pb-4 px-1 border-b-2 font-medium text-sm transition-colors ${
                    currentSection === section.key
                      ...
                      : 'border-transparent text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300 hover:border-gray-300 dark:hover:border-white/20'
                  }`}
                >
          ...
          {/* Section Content */}
          <div className="mb-8">
            {currentSection === 'identity' && renderIdentitySection()}
            {currentSection === 'goals' && renderGoalsSection()}
            {currentSection === 'schedule' && renderScheduleSection()}
            {currentSection === 'energy' && renderEnergySection()}
          </div>
          ...
          <div className="flex justify-between items-center pt-6 border-t border-gray-200 dark:border-white/10">
            ...
            <div className="flex gap-3">
              ...
              {/* Next button - only show if not on last section */}
